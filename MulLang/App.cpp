...
class CDlgChild: public CWndImplBase
{
public:
    CDlgChild() { }
    void SetFocus(LPCTSTR pstrCtrl)
    {
        CControlUI *pCtrl = m_pm.FindControl(pstrCtrl);
        m_pm.SetFocusNeeded(pCtrl);
    }
protected:
    virtual CDuiString GetSkinFolder() { return _T("skin\\MulLang"); }
    virtual CDuiString GetSkinFile() { return _T("child.xml"); }
    virtual LPCTSTR GetWindowClassName(void) const { return _T("UIChild"); }
...
class CDlgLogin : public CWndImplBase
{
public:
    CDlgLogin() { }
...
protected:
    ...
    virtual LPCTSTR GetWindowClassName(void) const { return _T("UILogin"); }
    virtual void OnFinalMessage(HWND hWnd)
    {
        CWndImplBase::OnFinalMessage(hWnd);
        delete this;
...
private:
    ...
    void OnPrepare()
    {
        ...
        //CControlUI *pCtrl = m_pm.FindControl(_T("btnChgLang"));
        CControlUI *pCtrl = m_pm.FindControl(_T("btnChgLang"));
        ...
        // 子窗体
        MakeCtrlSizeNty(dynamic_cast<CContainerUI *>(m_pm.FindControl(_T("child"))));
        SetCurDlgType(_T("child"));
        CWndImplBase *pChild = new CDlgChild;
    ...
    void Notify(TNotifyUI &msg)
    {
        if (msg.sType == _T("click"))
        {
            ...
            else if (msg.pSender->GetName() == _T("btn_create_child_dlg"))
            {
                CWndImplBase *pDlg = new CDlgLogin;
                pDlg->Create(GetHWND(), _T("%[title]"), UI_WNDSTYLE_DIALOG, WS_EX_WINDOWEDGE);
                pDlg->CenterWindow();
                return;
            }
        }
