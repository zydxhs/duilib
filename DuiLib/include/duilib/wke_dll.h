...
#define WKE_CALL __cdecl
	...
	// wkeSetFileSystem: 是 wke 遗留接口，已被更高级的接口取
	...
	// 单独设置资源回收间隔
	DUILIB_API void wkeGC(wkeWebView webView, long intervalSec);
	...
	DUILIB_API void wkeSetSystemTouchEnabled(wkeWebView webView, bool b);
	...
	DUILIB_API void wkeUnlockViewDC(wkeWebView webView);
	...
	DUILIB_API bool wkeNavigateAtIndex(wkeWebView webView, int index);
	DUILIB_API int wkeGetNavigateIndex(wkeWebView webView);
	...
	DUILIB_API wkeRect *wkeGetCaretRect2(wkeWebView webView);
	...
	DUILIB_API void wkeEnableHighDPISupport();
...
#define JS_CALL __fastcall
	...
	DUILIB_API CDuiString jsToDoubleStr(jsExecState es, jsValue v);
	...
	DUILIB_API jsValue jsDouble(double v);
	DUILIB_API jsValue jsDoubleStr(const CDuiString &v);
	DUILIB_API jsValue jsBoolean(bool v);
