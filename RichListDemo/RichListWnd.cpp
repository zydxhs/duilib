...
void CPage1::OnClick(TNotifyUI &msg)
{
    ...
    {
        CControlUI *find_ctrl = m_pPaintManager->FindSubControlByName(msg.pSender->GetParent()->GetParent(),
                                _T("down_name"));
...
DUI_BEGIN_MESSAGE_MAP(CRichListWnd, CWndImplBase)
DUI_ON_MSGTYPE(DUI_MSGTYPE_CLICK, OnClick)
DUI_ON_MSGTYPE(DUI_MSGTYPE_SELECTCHANGED, OnSelectChanged)
DUI_ON_MSGTYPE(DUI_MSGTYPE_ITEMCLICK, OnItemClick)
DUI_END_MESSAGE_MAP()
...
LRESULT CRichListWnd::OnSysCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled)
{
    ...
    if (::IsZoomed(*this) != bZoomed)
    {
        if (!bZoomed)
        {
            ...
            pControl = static_cast<CControlUI *>(m_pm.FindControl(_T("btnrestore")));
...
void CRichListWnd::OnInitWindow()
{
    m_pCloseBtn = static_cast<CButtonUI *>(m_pm.FindControl(_T("btnclose")));
    m_pMaxBtn = static_cast<CButtonUI *>(m_pm.FindControl(_T("btnmax")));
    m_pRestoreBtn = static_cast<CButtonUI *>(m_pm.FindControl(_T("btnrestore")));
...
LRESULT CRichListWnd::OnMouseHover(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled)
{
    ...
    {
        MessageBox(NULL, _T("鼠标在某控件例如按钮上悬停后，对目标控件操作，这里改变了状态图标大小"),
                   _T("DUILIB DEMO"), MB_OK);
